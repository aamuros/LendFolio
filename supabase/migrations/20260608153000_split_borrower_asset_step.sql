alter table public.borrower_portfolios
  add column if not exists asset_declaration_completed boolean not null default false;

update public.borrower_portfolios
set existing_debt_declaration_completed = true
where not existing_debt_declaration_completed
  and (
    coalesce(has_existing_debts, false)
    or coalesce(existing_loan_payments, 0) > 0
    or coalesce(personal_loan_payments, 0) > 0
    or coalesce(business_loan_payments, 0) > 0
    or coalesce(vehicle_loan_payments, 0) > 0
    or coalesce(home_loan_payments, 0) > 0
    or coalesce(lending_app_payments, 0) > 0
    or coalesce(informal_loan_payments, 0) > 0
    or coalesce(buy_now_pay_later_payments, 0) > 0
    or coalesce(credit_card_payments, 0) > 0
    or coalesce(co_maker_guaranteed_loan_payments, 0) > 0
    or coalesce(other_debt_payments, 0) > 0
  );

update public.borrower_portfolios
set asset_declaration_completed = true
where not asset_declaration_completed
  and (
    coalesce(existing_debt_declaration_completed, false)
    or coalesce(cash_on_hand, 0) > 0
    or coalesce(bank_savings, 0) > 0
    or coalesce(ewallet_balance, 0) > 0
    or coalesce(inventory_value, 0) > 0
    or coalesce(business_equipment_value, 0) > 0
    or coalesce(vehicle_value, 0) > 0
    or coalesce(property_land_value, 0) > 0
    or coalesce(other_assets_value, 0) > 0
  );
