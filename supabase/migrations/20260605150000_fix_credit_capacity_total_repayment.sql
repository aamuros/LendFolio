-- Fix credit capacity enforcement to account for total repayment (principal + interest + fees).
-- 1. Restore repayment_amount <= available_credit_at_submission check in create_loan_offer.
-- 2. Fix accept_loan_offer to check repayment_amount (not just approved_amount) against credit limit.

-- 1. Restore the total-repayment credit-capacity check in create_loan_offer.
...
begin
  ...
  if p_repayment_amount > v_available_credit then
    return jsonb_build_object(
      'ok', false,
      'message', 'Total repayment (principal + interest + fees) cannot exceed the borrower''s available credit of PHP ' ||
        to_char(v_available_credit, 'FM999,999,999') || '. Reduce the approved amount, interest, or fees.'
    );
  end if;
  ...
  return jsonb_build_object(
    ...
    'remaining_credit_after_total_repayment',
      round(v_available_credit - p_repayment_amount, 2)
  );
exception
...
begin
  ...
  if found then
    ...
    if v_offer.repayment_amount > v_available_credit then
      return jsonb_build_object(
        'ok', false,
        'message', 'Accepting this offer would exceed your credit limit. The total repayment of PHP ' ||
          to_char(v_offer.repayment_amount, 'FM999,999,999') || ' exceeds your available credit of PHP ' ||
