create or replace function app_private.try_create_notification(
  p_user_id uuid,
  p_type text,
  p_title text,
  p_message text,
...
exception
  when others then
    raise warning 'Notification creation failed for user %, type %: %',
      p_user_id,
      p_type,
      sqlerrm;
end;
...
revoke all on function app_private.try_create_notification(
  uuid,
  text,
  text,
  text,
