-- Fix offer credit capacity enforcement to check principal only.
-- Interest/service charges and fees remain part of repayment, not credit capacity.

alter table public.loan_offers
  add column if not exists interest_service_charge_rate numeric(6, 3);
...
declare
  ...
  v_recomputed_repayment_amount numeric(12, 2);
...
begin
  ...
  then
    return jsonb_build_object(
      'ok', false,
      'message', 'Approved principal must be greater than PHP 0 and PHP 1,000,000 or less.'
    );
  end if;

  if p_repayment_amount is null
    or p_repayment_amount <= 0
    or p_repayment_amount > 1500000
  then
    return jsonb_build_object(
      'ok', false,
      'message', 'Repayment amount must be greater than PHP 0 and PHP 1,500,000 or less.'
    );
  end if;

  if p_interest_service_charge_rate is null
    or p_interest_service_charge_rate < 0
    or p_interest_service_charge_rate > 100
  then
    return jsonb_build_object(
      'ok', false,
      'message', 'Interest or service charge rate must be between 0% and 100%.'
    );
  end if;
  ...
  v_implied_interest_amount :=
    round(round(p_approved_amount, 2) * (p_interest_service_charge_rate / 100), 2);
  v_recomputed_repayment_amount :=
    round(p_approved_amount, 2) + v_implied_interest_amount + round(p_fees, 2);
  ...
  v_available_credit := coalesce(
    v_application.available_credit_at_submission,
  ...
  return jsonb_build_object(
    ...
    'total_repayment_amount', v_recomputed_repayment_amount,
    'remaining_credit_after_principal',
      round(v_available_credit - p_approved_amount, 2)
  );
exception
...
-- 2. Fix accept_loan_offer to check repayment_amount (total cost) not approved_amount (principal only).
...
grant execute on function app_private.create_loan_offer(
  uuid, numeric, numeric, numeric, numeric, date, text, text, text, text, text
) to authenticated;
