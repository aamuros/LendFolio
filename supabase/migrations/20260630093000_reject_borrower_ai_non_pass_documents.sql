...
declare
  ...
  v_document_status public.borrower_verification_document_status := 'rejected';
...
begin
  ...
  values (
    ...
    case
      ...
      when v_ai_review_status = 'needs_manual_review' then 'AI review could not confidently accept this upload. Please upload a clearer document.'
  ...
  return jsonb_build_object(
    ...
    'message', case
      ...
      else 'Verification document needs replacement.'
    end,
...
with changed_documents as (
  ...
  set
    status = 'rejected',
    review_notes = case
      when ai_review_status = 'fail' then 'AI review could not accept this upload. Please upload the correct document.'
  ...
  returning borrower_verification_id
)
select app_private.refresh_borrower_verification_submission_state(
  borrower_verification_id
)
from (
  select distinct borrower_verification_id
  from changed_documents
) changed_verifications;
