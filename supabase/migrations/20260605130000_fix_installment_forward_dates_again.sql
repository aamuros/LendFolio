-- Restore forward-date repayment schedule logic.
-- The add_repayment_destination migration accidentally reverted the installment
-- date calculation back to backward-from-offer-due-date, which can produce
-- dates in the past when the offer due date is close to the acceptance date.
-- This restores the forward logic: installment N is due on current_date + N months.
