-- Restore offer workflow notifications that were lost when later migrations
-- replaced create_loan_offer and accept_loan_offer without carrying over
-- the try_create_notification calls.

-- 1. Restore offer_received notification in create_loan_offer.
...
-- 2. Restore offer_accepted and offer_declined notifications in accept_loan_offer.
...
declare
  ...
  v_declined_lender_ids uuid[] := array[]::uuid[];
...
begin
  ...
  v_loan_due_date := (current_date::date + (v_installment_count || ' months')::interval)::date;

  -- Collect competing lender IDs before declining them.
  select array_agg(lender_id)
  ...
  for v_installment_number in 1..v_installment_count loop
    ...
    v_installment_due_date :=
      (current_date::date + (v_installment_number || ' months')::interval)::date;
  ...
  -- Notify the lender whose offer was accepted.
  perform app_private.try_create_notification(
    v_offer.lender_id,
    'offer_accepted',
    'Offer accepted',
  ...
  -- Notify competing lenders whose offers were declined.
  if v_declined_lender_ids is not null then
