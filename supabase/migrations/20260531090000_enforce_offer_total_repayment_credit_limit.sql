-- Enforce that total repayment does not exceed borrower available credit at submission.
-- Borrower credit capacity caps total repayment obligation, not only approved principal.
...
begin
  ...
  if p_repayment_amount > v_available_credit then
    return jsonb_build_object(
      'ok', false,
      'message', 'Total repayment cannot exceed the borrower''s available credit at submission.'
    );
  end if;
